...
function calc() {
  //// Кулькулятор ////
  // Инпут для количества человек 
  var persons = document.querySelectorAll('.counter-block-input')[0],
      // Инпут для количества дней
  restDays = document.querySelectorAll('.counter-block-input')[1],
      // Селектор для выбора места 
  place = document.getElementById('select'),
      // Div с общей стоимостью
  totalValue = document.getElementById('total'),
      // Сумма количества человек
  personsSum = 0,
      // Сумма количества дней
  daysSum = 0,
      // Общая сумма, в сложении всех параметров
  total = 0; // Обнуляем значение поля общей суммы

  totalValue.innerHTML = 0; // Событие на первый инпут с людьми

  persons.addEventListener('input', function () {
    // Записываем сюда данные с инпута
    personsSum = +this.value; // Запрещаем все буквы и символы

    this.value = this.value.replace(/^[A-Z]+$/g, ''); // Формула расчета - Сумма берется от заказчика и она сейчас рандомна

    total = (daysSum + personsSum) * 5000; // Если 2 поле с днями пустое, то мы не расчитываем сумму
    ...
    return personsSum;
  }); // Событие на второй инпут с днями

  restDays.addEventListener('input', function () {
    // Записываем сюда данные с инпута
    daysSum = +this.value; // Формула расчета - Сумма берется от заказчика и она сейчас рандомна

    total = (daysSum + personsSum) * 5000; // Запрещаем все буквы и символы

    this.value = this.value.replace(/^[A-Z]+$/g, ''); // Если 1 поле с людьми пустое, то мы не расчитываем сумму
    ...
    return daysSum;
  }); // Функция сбрасывания общей стоимости, если один из инпутов пустой

  persons.addEventListener('input', function () {
    var a = this.value;
    var myString = a.toString();
    var newArray = myString.slice(0, 1);
  ...
  }); // Функция выбора города, с расчетом стоимости

  place.addEventListener('change', function () {
    // Проверям заполненность полей, если нет, то сбрасываем общюю стоимость
    if (restDays.value == '' || persons.value == '') {
      totalValue.innerHTML = 0;
    } else {
      // Передаем сюда переменную total, сохранив ее значение для расчетов
      var a = total; // Находим select и выбираем его значение
...
function form() {
  ...
  var contactForm = document.getElementById('form'),
      contactMail = contactForm.querySelectorAll('input')[0],
      contactPhone = contactForm.querySelectorAll('input')[1],
  ...
  contactPhone.addEventListener('input', function () {
    ...
    if (this.value.length > 10) {
      console.log('Слишком много цифр'); // Здесь будет проверка
    }
  });
...
function formModal() {
  ...
  var message = {
    loading: 'Загрузка...',
    // При загрузке
    success: 'Спасибо, скоро мы с вами свяжемся! Хорошего дня!',
    // При Если все хорошо
    failure: 'Что-то пошло не так...' // Если ошибка

  };
  var formModal = document.querySelector('.main-form'),
      // Сама форма
  inputModal = formModal.getElementsByTagName('input'),
      // Поле ввода
  statusMessage = document.createElement('div'); // Создаем новый div
  // Присваиваем новому диву заранее созданный класс

  statusMessage.classList.add('status'); // Проверяю инпут, запрет на буквы, только цифры

  inputModal[0].addEventListener('input', function () {
    this.value = this.value.replace(/[^\d(?:+)]/ig, '');
...
function modal() {
  //// Модальное окно на табах ////
  var overlay = document.querySelector('.overlay'),
      // Само модальное окно
  more = document.querySelector('.more'),
      // Кнопка вызова
  body = document.querySelector('body'); // Кнопка вызова

  var modalWindow = function modalWindow(event) {
  ...
  body.addEventListener('click', modalWindow);
}
...
function slider() {
  ...
  var slideIndex = 1,
      // Сами слайды - фото, картинки
  slides = document.querySelectorAll('.slider-item'),
      // Навигация вперед - стрелка
  prev = document.querySelector('.prev'),
      // Навигация назад - стрелка
  next = document.querySelector('.next'),
      // Div с точками для переключения
  dotsWrap = document.querySelector('.slider-dots'),
      // Точки для переключения 
  dots = document.querySelectorAll('.dot'); // Функция показа слайдов 

  showSlides(slideIndex); // Функция показа слайдов 
  ...
  function showSlides(n) {
    ...
    }); // Перебираем все точки под слайдером, и удаляем класс с подсветкой активной точки

    dots.forEach(function (item) {
      return item.classList.remove('dot-active');
    }); // Начали нумерацию с 0 и показали нужный слайдер из переменной

    slides[slideIndex - 1].style.display = 'block'; // Начали нумерацию точек с 0 и присвоили активный класс нужной
  ...
  } // Определяем текущий слайд и устанавливает его  


  function currentSlide(n) {
    // когда мы нажымаем на 4 точку, сюда передается 4 и нам показывает 4 слайдер
    showSlides(slideIndex = n);
  ...
  dotsWrap.addEventListener('click', function (event) {
    // Добавляем 1 к массиву, что бы прировнять число js к реальному числу 
    for (var i = 0; i < dots.length + 1; i++) {
      // Проверяем содержит ли кнопка класс dot и имеет ли индекс, переводя его в js / i-1
      if (event.target.classList.contains('dot') && event.target == dots[i - 1]) {
        // Устанавливаем слайд на нужную точку
        currentSlide(i);
      }
    }
  });
...
function timer() {
  ...
  var getTimeRemaining = function getTimeRemaining(endtime) {
    var t = Date.parse(endtime) - Date.parse(new Date()),
        ...
        hours = Math.floor(t / (1000 * 60 * 60)); // let days =  Math.floor( (t/(1000*60*60*24) )); Дни  
